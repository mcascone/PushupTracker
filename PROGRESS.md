...
Replaced the Xcode template scaffold to start M3. Rewrote `PushupTracker/PushupTrackerApp.swift` to construct the shared `ModelContainer` via `SharedContainer.makeModelContainer()` from `PushupCore` and inject it into the scene. Added `PushupTracker/AppShell.swift` — a four-tab `TabView` (Today, History, Trends, Settings) with placeholder `Text` views for the three non-Today tabs. Added `PushupTracker/Views/Today/TodayView.swift` as a stub (content for hero number, quick-add buttons, timeline, and undo banner deferred to subsequent M3 iterations to keep file count small). Deleted the template `Item.swift` and `ContentView.swift`. The project uses `PBXFileSystemSynchronizedRootGroup` so file additions/deletions need no pbxproj edits. SourceKit briefly flagged "No such module 'PushupCore'" on the app file (same spurious indexer behavior as M2's "Testing" diagnostic); the actual build is clean. `xcodebuild ... build` and `xcodebuild ... test` both succeed; `swift test --package-path PushupCore` still 8/8. First exit criterion (`AppShell` injects shared `ModelContainer`) is met; remaining M3 criteria still open.
